...
// Expanded lookups matrix dictionary to catch string mismatches automatically
const pageTitles = (name) => ({
  ...
  // ✅ Multi-key safety fallback nets to catch exactly what your sidebar emits:
  myassets:      "My Assets",
  myassetsview:  "My Assets",
  assets:        "My Assets",
  
  // 👥 ADDED: Directory lookups to safely catch the active view token
...
const Header = ({ activePage, onLogout, user }) => {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // SAFE ROUTING NORMALIZATION LOOKUP
  // ═══════════════════════════════════════════════════════════════════════════
  const cleanedPageKey = typeof activePage === 'string' 
    ? activePage.replace(/\s+/g, '').toLowerCase() 
    : '';

  const evaluatedHeaderTitle = pageTitles(currentUserName)[cleanedPageKey] || pageTitles(currentUserName)[activePage] || "CoreHR System";

  return (
    <header className={styles.topbar}>
      <h1 className={styles.pageTitle}>
        {evaluatedHeaderTitle}
      </h1>
      ...
      <div className={styles.userInfo}>
                  ...
                  announcements.map((ann) => (
                    ...
                    >
                      ...
                      <div className={styles.notificationItemDesc}>
                        {ann.description.length > 70 ? ann.description.substring(0, 70) + '...' : ann.description}
                      </div>
                      <div className={styles.notificationItemMeta}>
                        <span className={`${styles.notificationPriorityBadge} ${ann.priority === 'High' ? styles.priorityHigh : ann.priority === 'Medium' ? styles.priorityMedium : styles.priorityLow}`}>
                          {ann.priority}
                        </span>
        ...
        {onLogout ? (
          <button type="button" className={styles.logoutButton} onClick={onLogout} style={{
            border: '1px solid #dbe3ef', background: '#ffffff', color: '#334155', borderRadius: '999px',
            padding: '9px 15px', fontSize: '0.88rem', fontWeight: '600', cursor: 'pointer', marginLeft: '8px'
          }}>
            Sign out
          </button>
        ) : null}
      ...
      {/* Center Modal for Details */}
      {selectedAnnouncement && (
        <div className={styles.modalOverlay} style={{ display: 'flex', zIndex: 99999 }} onClick={() => setSelectedAnnouncement(null)}>
          <div className={styles.modalContent} style={{ maxWidth: '520px', padding: '28px', position: 'relative', overflow: 'hidden' }} onClick={(e) => e.stopPropagation()}>
            <button
              type="button"
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
              <span style={{ fontSize: '0.78rem', fontWeight: '700', textTransform: 'uppercase', color: '#4f46e5', backgroundColor: '#eef2ff', padding: '4px 10px', borderRadius: '6px' }}>
                {selectedAnnouncement.category || 'General'}
              </span>
              <span className={`${styles.notificationPriorityBadge} ${selectedAnnouncement.priority === 'High' ? styles.priorityHigh : selectedAnnouncement.priority === 'Medium' ? styles.priorityMedium : styles.priorityLow}`} style={{ padding: '4px 10px', borderRadius: '6px' }}>
                {selectedAnnouncement.priority} Priority
              </span>
            </div>

            <h2 style={{ fontSize: '1.4rem', fontWeight: '800', color: '#0f172a', lineHeight: '1.3', margin: '0 0 16px 0', letterSpacing: '-0.02em', textAlign: 'left' }}>
              {selectedAnnouncement.title}
            </h2>

            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', color: '#94a3b8', borderBottom: '1px solid #f1f5f9', paddingBottom: '14px', marginBottom: '20px' }}>
              <span>By: <strong>{selectedAnnouncement.sender?.name || 'System'}</strong></span>
              <span>{formatTime(selectedAnnouncement.createdAt)}</span>
            </div>

            <div style={{ fontSize: '0.95rem', color: '#334155', lineHeight: '1.6', whiteSpace: 'pre-line', maxHeight: '260px', overflowY: 'auto', paddingRight: '6px', textAlign: 'left' }}>
              {selectedAnnouncement.description}
            </div>

            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '24px', borderTop: '1px solid #f1f5f9', paddingTop: '16px' }}>
              <button type="button" className={styles.btnSubmit} style={{ borderRadius: '8px', padding: '10px 24px', background: '#4f46e5', color: '#fff', border: 'none', fontWeight: '600', cursor: 'pointer' }} onClick={() => setSelectedAnnouncement(null)}>
                Acknowledge
              </button>
            </div>
          </div>
